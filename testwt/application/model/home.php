...
class_exists("generic_class") ? "" : require_once( CONTROLLER_DIR . '/class.generic.php' );

if( !defined('DB_HOST') ) {
  require_once( ROOT_DIR . '/db/DBConfig.php');
}

class homeContent extends generic_class {  
  ...
  function newReleasesSql() {
    ...
    foreach ($nr['al_with_year'] as $value) {
      $nr['imgArray'][] = $this->get_images( "album", $value, "_175x175" );
    } 
...
WHERE t.rn <= 2 LIMIT 30";
    ...
    foreach ($fp['pl_title_index'] as $key => $value) {
      $fp['image_src'][] = $this->get_images( "playlist", $value, "_80x80" );
    }
