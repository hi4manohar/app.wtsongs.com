...
if( defined('DB_NAME') ) {
  ...
  require_once DB_DIR . "/DBConfig.php";
}
...
class commonClass {
  ...
  function is_track_exist_in_album( $track, $album ) {
    $tr_exist = mysqli_query( $GLOBALS['link'], "SELECT al.album_id, wt.track_id FROM wt_albums as al, wt_tracks as wt WHERE wt.track_title='" . $track . "' AND al.album_title='" . $album . "' LIMIT 1" );
    if( mysqli_num_rows( $tr_exist ) == 1 ) {
      while( $tDataRow = mysqli_fetch_assoc( $tr_exist ) ) {
        $data['track_id'] = $tDataRow['track_id'];
        $data['album_id'] = $tDataRow['album_id'];
      }
      return $data;
    } else return false;
  }
  ...
  //execute query and return true if number_of_rows greater than 1
  function get_query_row($data) {
    $ex_query = mysqli_query( $GLOBALS['link'], $data );
    if( $this->checkquery($ex_query) ) {
